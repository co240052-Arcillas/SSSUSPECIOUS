...
public class Meat : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Finds the Snake object and gets the Meat's collider, with a warning if missing
        snake = FindFirstObjectByType<Snake>();
        col = GetComponent<Collider2D>();
        if (col == null)
        {
            Debug.LogWarning("Meat has no Collider2D! Defaulting to 1x1 size.");
        }
    }

    private void OnEnable()
    {
        // Starts the coroutine to periodically relocate the meat
    ...
    {
        // Moves the meat to a random unoccupied, non-blocked position within the grid
        ...
        do
        {
            ...
            {
                Debug.LogWarning("Could not find free spot for Meat after 100 attempts!");
                break;
            }
    ...
    {
        // Continuously relocates the meat every 'lifeTime' seconds
        while (true)
        {
            Relocate();
